...
class bot_mapper():
    ...
    def connect_neighbors(self,maze,node_row,node_col,case,step_l = 1,step_up = 0,totl_cnncted = 0):
        ...
        if (maze[node_row - step_up][node_col - step_l]>0):
            ...
            # No path in this direction, search in the next direction
            if not self.connected_left:
                # Wall on left, look up lft
                self.connected_left = True
                # Looking upleft now
                step_l = 1
                step_up = 1
    ...
    def get_surround_pixel_intensities(maze,curr_row,curr_col):
        ...
        # Extract surround pixel intensities and Address boundary conditions (if present)
        ...
        # If the point we are at is on the top row, its top pixel is not accesible
        if (top_row):
            top = 0
        else:
    ...
    def reset_connct_paramtrs(self):
        # Reset member variables to False initially when looking for nodes to connect
        self.connected_left = False
        self.connected_upleft = False
        self.connected_up = False
        self.connected_upright = False
    ...
    def one_pass(self,maze):
        ...
        for row in range(rows):
            for col in range(cols):
                if (maze[row][col]==255):
                    if debug_mapping:
                        # Reinitalizize Maze_connect with Colored Maze
                        self.maze_connect = cv2.cvtColor(maze, cv2.COLOR_GRAY2BGR)
    ...
    def graphify(self,extracted_maze):
        ...
        if not self.graphified:
            # Peform thinning on maze, paths that car could follow
            thinned = cv2.ximgproc.thinning(extracted_maze)
            # Dilate, thin again to minimize unneccesary interest point (i.e:turns)
            ...
            #Crop out Boundary not part of maze
            ...
            # Identify Interest Points in the path
