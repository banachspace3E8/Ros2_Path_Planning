...
class bot_localizer():
    ...
    def extract_bg(self, frame):
        ...
        if min_cntr_idx != -1:
            ...
            #Area tha doesnt include cars
            NotCar_mask = cv2.bitwise_not(car_mask)
            frame_car_remvd = cv2.bitwise_and(frame, frame, maxk = NotCar_mask)
            ...
            self.bg_model = cv2.bitwise_or(Ground_replica, frame_car_remvd)
        ...
        cv2.imshow('1d. Background_Moel', self.bg_model)
    ...
    def get_car_loc(self, car_cnt, car_mask):
        ...
        bot_cntr_arr = np.arr([bot_cntr[1], bot_cntr[0]])
    ...
    def localize_bot(self, curr_frame, frame_disp):
        ...
        car_circular_mask = cv2.circle(car_mask.copy(), (int(center[0], int(center[1])), int(radii)))
        ...
        #Displaying extracted car mask and localized car in frame
        cv2.imshow("foreground(car)", car_mask)
        cv2.imshow("foreground(car)", frame_disp)
